...
install: all
	install -d $(DESTDIR)$(prefix)/lib $(DESTDIR)$(prefix)/include/rate_adjusting_pcm_ring
	install -m 0644 build/librate_adjusting_pcm_ring.a $(DESTDIR)$(prefix)/lib/
	install -m 0755 build/librate_adjusting_pcm_ring.so.0.1.0 $(DESTDIR)$(prefix)/lib/
	ln -sf librate_adjusting_pcm_ring.so.0.1.0 $(DESTDIR)$(prefix)/lib/librate_adjusting_pcm_ring.so.0
	ln -sf librate_adjusting_pcm_ring.so.0 $(DESTDIR)$(prefix)/lib/librate_adjusting_pcm_ring.so
...
install-check: all
	...
	$(CC) $(WARNINGS) $(CONSUMER_TEST) -Ibuild/stage/usr/include/rate_adjusting_pcm_ring -Lbuild/stage/usr/lib -Wl,-rpath,$(CURDIR)/build/stage/usr/lib -lrate_adjusting_pcm_ring -o build/test_consumer
	./build/test_consumer
distcheck: install-check
	rm -rf build/distcheck
	mkdir -p build/distcheck/rate_adjusting_pcm_ring-$(VERSION)
	cp -a Makefile Doxyfile README.md QUALITY.md AGENTS.md include src tests build/distcheck/rate_adjusting_pcm_ring-$(VERSION)/
